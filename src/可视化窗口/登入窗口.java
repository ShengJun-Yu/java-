...
class keyHandler implements KeyListener {
 ...
 public void keyPressed(KeyEvent e) {
  ...
  // 显示输入的用户名
  if ((ob == 登入窗口.txtName) && (e.getKeyCode() == 10)) {
   System.out.println(登入窗口.txtName.getText());
  }
  // 如果事件对象为txtPass，并且按下回车键，则在控制台中显示输入的密码
  else if ((ob == 登入窗口.txtPass) && (e.getKeyCode() == 10)) {
   System.out.println(登入窗口.txtPass.getText());
  }
 }
 // 释放某个键时调用此方法
